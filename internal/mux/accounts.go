...
type AccountSnapshot struct {
	Usage           *provideradapter.Usage `json:"usage,omitempty"`
		...
		go func(account state.Account) {
			...
			if err != nil {
				snapshot = AccountSnapshot{
					ID: account.ID, Label: account.Label, Kind: account.Kind, Provider: account.Provider, Model: account.Model, Enabled: account.Enabled,
...
func (m *Multiplexer) accountSnapshotWithProfile(ctx context.Context, accountID string, includeProfile bool) (AccountSnapshot, error) {
	...
	if account.Kind == "provider" {
		snapshot := AccountSnapshot{ID: account.ID, Label: account.Label, Kind: account.Kind, Provider: account.Provider, Model: account.Model, Enabled: account.Enabled, Connected: true, AuthType: "provider", CreatedAt: account.CreatedAt, ThreadCount: m.store.ThreadCounts()[account.ID], PlanLabel: "API provider"}
		if includeProfile {
			snapshot.Usage = m.providerUsage(ctx, account)
		}
		return snapshot, nil
	}
...
func (m *Multiplexer) chooseAccountExcluding(ctx context.Context, excluded map[string]struct{}) (state.Account, RouteReason, error) {
	...
	for _, snapshot := range snapshots {
		if m.coolingDown(snapshot.ID) {
			continue
		}
		if _, skip := excluded[snapshot.ID]; skip {
			continue
		}
		if !snapshot.Enabled || !snapshot.Connected || (snapshot.AuthType != "chatgpt" && snapshot.AuthType != "provider") {
			continue
		}
		...
		if (weekly != nil && weekly.UsedPercent >= 100) || (short != nil && short.UsedPercent >= 100) {
			continue
		}
		weeklyUsed := 1_000.0
		shortUsed := 1_000.0
	...
	for index := range candidates {
		go func(index int, account state.Account) {
			resetResults <- resetResult{
				index: index, metadata: func() resetCreditMetadata {
					if account.Kind == "provider" {
						return resetCreditMetadata{}
					}
					return m.routingResetCredits(ctx, account)
				}(),
			}
