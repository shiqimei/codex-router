...
import (
	...
	"github.com/shiqimei/codex-router/internal/mux"
...
func New(address, token string, multiplexer *mux.Multiplexer, uiTests bool) *Server {
	...
	router.HandleFunc("/v1/providers", server.providers)
	router.HandleFunc("/v1/providers/", server.providerAction)
	router.HandleFunc("/v1/routing", server.routing)
	router.HandleFunc("/v1/model-context", server.modelContext)
	router.HandleFunc("/v1/thread-switch", server.switchThread)
...
func (s *Server) providers(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	account, err := s.mux.AddProvider(ctx, input)
	if err != nil {
		writeJSON(w, 400, map[string]any{"error": err.Error()})
		return
	}
	writeJSON(w, 201, map[string]any{"account": account})
}
func (s *Server) switchThread(w http.ResponseWriter, r *http.Request) {
	if !s.authorized(r) {
		writeJSON(w, 401, map[string]any{"error": "unauthorized"})
		return
	}
	...
	defer cancel()
	result, err := s.mux.SwitchThread(ctx, input)
	if err != nil {
		writeJSON(w, 409, map[string]any{"error": err.Error(), "result": result})
		return
	}
...
func (s *Server) routing(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case "GET":
		writeJSON(w, 200, map[string]any{"defaultAccountId": s.mux.DefaultAccount()})
	case "POST":
		var input struct {
			ID string `json:"defaultAccountId"`
		}
		if err := decodeJSON(r, &input); err != nil {
			writeJSON(w, 400, map[string]any{"error": err.Error()})
			return
		}
		if err := s.mux.SetDefaultAccount(input.ID); err != nil {
			writeJSON(w, 400, map[string]any{"error": err.Error()})
			return
		}
		writeJSON(w, 200, map[string]any{"defaultAccountId": input.ID})
	default:
		methodNotAllowed(w)
	}
}
...
func (s *Server) providerAction(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 2 && parts[1] == "restore" && r.Method == "POST" {
		account, err := s.mux.RestoreProvider(ctx, id)
		if err != nil {
	...
	switch r.Method {
	case "GET":
		details, err := s.mux.ProviderDetails(id)
		if err != nil {
	...
	case "PUT":
		var input state.ProviderUpdate
		if err := decodeJSON(r, &input); err != nil {
			writeJSON(w, 400, map[string]any{"error": err.Error()})
			return
		}
		account, err := s.mux.UpdateProvider(ctx, id, input)
		if err != nil {
			writeJSON(w, 409, map[string]any{"error": err.Error()})
			return
		}
		writeJSON(w, 200, map[string]any{"account": account})
	case "DELETE":
		if err := s.mux.DeleteProvider(ctx, id); err != nil {
			writeJSON(w, 409, map[string]any{"error": err.Error()})
			return
		}
		writeJSON(w, 200, map[string]any{"deleted": true, "undoAvailable": true})
	default:
		methodNotAllowed(w)
	}
}

func (s *Server) modelContext(w http.ResponseWriter, r *http.Request) {
	if !s.authorized(r) {
		writeJSON(w, 401, map[string]any{"error": "unauthorized"})
		return
	}
	...
	value, err := s.mux.ModelContext(r.URL.Query().Get("threadId"))
	if err != nil {
		writeJSON(w, 404, map[string]any{"error": err.Error()})
		return
	}
